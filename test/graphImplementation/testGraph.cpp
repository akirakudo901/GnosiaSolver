...
struct F {
    ...
    :  g( new Graph() )
    {
        vv1 = new VariableVertex("vv1", initDomain);
        vv2 = new VariableVertex("vv2", initDomain);
        vv3 = new VariableVertex("vv3", initDomain);
...
BOOST_FIXTURE_TEST_SUITE(Graph_test_suite, F, * boost::unit_test::label("Graph"));
    ...
    BOOST_AUTO_TEST_SUITE(test_contains_vertex);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            ...
            BOOST_TEST(g->contains_vertex(nullptr) == false);
        };
    ...
    BOOST_AUTO_TEST_SUITE(test_adjacent);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            ...
            BOOST_TEST(g->adjacent(vv1, nullptr) == false);
            BOOST_TEST(g->adjacent(nullptr, cv1) == false);
            BOOST_TEST(g->adjacent(nullptr, nullptr) == false);
        }
    ...
    BOOST_AUTO_TEST_SUITE(test_neighbors);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            // test against nullptrs
            BOOST_REQUIRE_EQUAL(g->neighbors(nullptr).size(), 0);
        };
    ...
    BOOST_AUTO_TEST_SUITE(test_add_vertex);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            // initial check: no vertices are in the graph
            BOOST_TEST_REQUIRE(g->contains_vertex(vv1) == false);
            BOOST_TEST_REQUIRE(g->contains_vertex(vv2) == false);
            BOOST_TEST_REQUIRE(g->contains_vertex(cv1) == false);
            
            // setup: add a nullptr vertex
            g->add_vertex(nullptr);
    ...
    BOOST_AUTO_TEST_SUITE(test_remove_vertex);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            ...
            // setup: remove a nullptr vertex
            g->remove_vertex(nullptr);
    ...
    BOOST_AUTO_TEST_SUITE(test_add_edge);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            ...
            // setup: add edges to the graph where one or more vertices are nullptrs
            // for now, having a nullptr edge passed does not affect the result
            g->add_edge(nullptr, vv2, *e);
            g->add_edge(vv1, nullptr, *e);
            g->add_edge(nullptr, nullptr, *e);
    ...
    BOOST_AUTO_TEST_SUITE(test_remove_edge);
        ...
        BOOST_AUTO_TEST_CASE(pass_nullptr) {
            ...
            // setup: remove edges with nullptr arguments
            g->remove_edge(nullptr, vv2);
            g->remove_edge(vv1, nullptr);
            g->remove_edge(nullptr, nullptr);
